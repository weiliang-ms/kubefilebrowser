...
<body>
	...
	<div id="containers">
		<table cellpadding="0" cellspacing="0" border="0" class="ge_table">
			...
			<tr>
				...
				<td>
					<select id="container" class="short_select" multiple="multiple" style="height:300px" size="30"></select>
                    <script type="text/javascript">
                      $("#container").select2({
                        placeholder: '请选择容器，最多选择20个',
                        allowClear: true,
	...
	<script>
		...
		function fileFolderOne() {
			...
			$("#container option:selected").each(function(index, obj) {
				containerArray.push($(obj).text());
			});
			var containerStr=containerArray.join(",");
			//新建一个FormData对象
			...
			$.ajax({
				type: 'POST',
				url: "/api/k8s/upload?namespace="+namespace.text()+"&pod_name="+pod.text()+"&container_name="+containerStr+"&dest_path="+destPath,
				timeout: 300 * 1000,
				data: formData,
				processData: false,
				contentType: false,
